...
// ⚠ the run goes through admin/test-lib/harness (Damon 2026-09-13, HARD
// commit gate): only a measured run prints the breakdown row + HARNESS-RUN
// line that arms the gate. The checks below are synchronous, so they run
// inside main() and feed the harness assert by assert.
const { harness } = require('/home/damon/platform/admin/test-lib/harness');

let pass = 0, fail = 0, H = null;
function t(name, cond, detail) {
  const ok = !!cond;
  if (ok) pass++; else fail++;
  if (H) H.assert(name, ok, detail);
  console.log((ok ? 'ok - ' : 'FAIL - ') + name + (!ok && detail ? ' — ' + detail : ''));
}

async function main() {
  H = await harness({ test: 'umbrel-store-validate', repo: 'umbrel-depool-store', longRun: true });
  ...
  await H.finish({ pass: fail === 0, extra: { tally: pass + ' passed, ' + fail + ' failed' } });
  process.exit(fail ? 1 : 0);
}

main().catch(async (e) => { console.error('validate: ' + (e && e.stack || e)); if (H) await H.finish({ pass: false }); process.exit(1); });
